...
list_num_nodes = [[32, 32], [32, 16], [32, 8], [16, 8], [16, 4], [8, 4], [32, 16, 8], [24, 16, 6]]
...
pmf_cv_results = pd.DataFrame(parameters)
pmf_cv_results["cindex"] = 0


kf = KFold(n_splits = 5)


for index in range(pmf_cv_results.shape[0]):
    num_nodes = pmf_cv_results.iloc[index, 0]
    batch_norm = pmf_cv_results.iloc[index, 1]
    dropout = pmf_cv_results.iloc[index, 2]
    batch_size = pmf_cv_results.iloc[index, 3]
    lr = pmf_cv_results.iloc[index, 4]
    ...
    for train_index, test_index in kf.split(df_train):
        ...
        ev = EvalSurv(surv, df_train["time"][test_index].values,
                      df_train["os"][test_index].values, censor_surv='km')
        c_index = ev.concordance_td('antolini')
        cindexes.append(c_index)

    pmf_cv_results.iloc[index, 5] = np.mean(cindexes)
    pmf_cv_results.to_csv('./data/cv.results.pmf.csv', index = False)

    print(pmf_cv_results.iloc[index, ])


pmf_cv_results = pd.read_csv("./data/cv.results.pmf.csv")
print(pmf_cv_results["cindex"].values.max())
ind_best = pmf_cv_results["cindex"].values.argmax()
num_nodes = eval(pmf_cv_results.iloc[ind_best, 0])
batch_norm = pmf_cv_results.iloc[ind_best, 1]
dropout = pmf_cv_results.iloc[ind_best, 2]
batch_size = pmf_cv_results.iloc[ind_best, 3]
lr = pmf_cv_results.iloc[ind_best, 4]
...
pd.DataFrame(data = {"cindex": c_indexes, "ibs": ibss}).to_csv("./data/results.ci.pmf.csv", index=False)
